...
export const getSampleQueue = (bullmqService?: BullMQService): Queue<SampleJobData, SampleJobResult> => {
  if (!sampleQueueInstance) {
    const service = bullmqService || new BullMQService();
...
export const initSampleWorker = (bullmqService?: BullMQService): Worker<SampleJobData, SampleJobResult> => {
